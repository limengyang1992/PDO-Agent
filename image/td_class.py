...
def generate_td_json(path,save_dir,num_classes=5,num_samples=10):
    ...
    for c in range(num_classes):
        ...
        model_predict = model_predict = np.random.normal(2,1.5,num_samples*num_classes).reshape(num_samples,num_classes)
        ...
        grad = torch.norm(1- model_possible,dim=1)    
        ...
        result_class_list.append(result_class)

    result = dict(zip(range(num_classes),result_class_list))
    result = {"td":result}
...
if __name__ == "__main__":
    import glob
    save_dir = "dataset/tasks_td_class/"
    jsons = glob.glob("dataset/tasks/*.json")
    for path in jsons[:20]:
        generate_td_json(path,save_dir)
